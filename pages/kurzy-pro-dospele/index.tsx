...
const KurzyProDospele = ({ courses, onlineCoursesRes, closedOldCourses, preparingCourses }: Props) => {
  const title = `Kurzy pro dospělé`
  const description = `Nabízíme kurzy pro dospělé, tak kroužky pro děti.`
  ...
  return (
    <>
      <NextSeo
        ...
        canonical={`${webUrl}/kurzy-pro-dospele`}
        openGraph={{
          url: `${webUrl}/kurzy-pro-dospele`,
          title,
          description,
          images: [
            {
        ...
        <SinglePage title={title}>
          <section
            ...
            {onlineCoursesRes.length > 0 && (
              <div>
                <H2 level={2}>Nabídka online kurzů pro dospělé</H2>
                <CoursesList
            ...
            {courses.length > 0 && (
              <div>
                <H2 level={2}>Nabídka prezenčních kurzů pro dospělé</H2>
                <CoursesList
                  courses={courses}
                  css={css`
                    margin: 20px 0 0 0;
          ...
          <CardRow>
            ...
            <Typography.Text>
              Cílem našich volnočasových aktivit je podpoření zájmu IT vzdělání. Nabízíme kurzy pro dospělé se zaměřením na programování v Pythonu, stavění
              chytrých řešení otevřené platformy Arduino, principy fungování neuronových sítí a 3D tisku.
            </Typography.Text>
          </CardRow>
...
export const getStaticProps: GetStaticProps = async (): Promise<GetStaticPropsResult<Props>> => {
  const coursesAdultRes = await getCourseList({ ageGroup: CourseAgeGroup.adult })

  const openInPersonCoursesRes = (coursesAdultRes.results || []).filter((d) => isCourseRegistrationOpen(d) && d.data.type === CourseType.inPerson)
  const openOnlineCoursesRes = (coursesAdultRes.results || []).filter((d) => isCourseRegistrationOpen(d) && d.data.type === CourseType.online)

  const closedOldAdultCourses = orderBy(
    coursesAdultRes.results.filter((d) => !isCourseRegistrationOpen(d)),
    "first_publication_date",
    "desc"
  )
  const preparingAllAdultCourses = orderBy(
    coursesAdultRes.results.filter((d) => isCourseRegistrationBefore(d)),
    "first_publication_date",
    "desc"
  )
  ...
  return {
    props: {
      ...
      closedOldCourses: closedOldAdultCourses || [],
      preparingCourses: preparingAllAdultCourses || []
    },
    revalidate: dev ? 1 : 60
  }
}

export default KurzyProDospele
